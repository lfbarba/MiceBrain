...
class BrainSliceConditionalDiffusion(nn.Module):
    ...
    def inpainting(self, pred_x_0, integer_brain_coord, brain2_slices_rs):
        with torch.no_grad():
    ...
    def step(
        ...
        timestep = torch.LongTensor([t]).to(self.device)
        ...
        with torch.no_grad():
            ...
            unfolded_conditioning = axial_conditioning.unfold(dimension=0, size=batch_size, step=batch_size).permute(0,
                                                                                                                     3,
                                                                                                                     1,
                                                                                                                     2)
            ...
            pred_x_0 = pred_x_0.roll([-x.item() for x in shifts], dims=[0, 1, 2])
            if with_inpainting:
                pred_x_0 = self.inpainting(pred_x_0, integer_brain_coord, brain2_slices_rs)
            return pred_x_0
